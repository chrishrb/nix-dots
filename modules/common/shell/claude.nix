{ config, pkgs, ... }:
let
  statusline-command = pkgs.writeShellScriptBin "statusline-command" ''
    input=$(cat)
    ...
    if [ -n "''$effort" ]; then
      printf "🤖 %s | 💪 %s | 🧠 %s | 💰 %s | 🕜 %s | 🌿 %s" "''$model" "''$effort" "''$usage_str" "''$block_str" "''$rate_limit_str" "''$git_str"
    else
      printf "🤖 %s | 🧠 %s | 💰 %s | 🕜 %s | 🌿 %s" "''$model" "''$usage_str" "''$block_str" "''$rate_limit_str" "''$git_str"
    fi
  '';
in
{
  home-manager.users.${config.user} = {
    programs.claude-code = {
      ...
      settings = {
        ...
        # No claude code co-authored commits
        attribution = {
          commit = "";
          pr = "";
        };
        ...
        statusLine = {
          command = "${statusline-command}/bin/statusline-command";
          padding = 0;
          type = "command";
        };
      };
